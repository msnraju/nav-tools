...
export default class TableFiltersReader {
    static readFilters(input: string): Array<IFilterCondition> {
        ...
        if (!stream.EOS)
            throw new Error(`Invalid Table Filters string '${input}'`);
    ...
    static read(stream: TokenStream): Array<IFilterCondition> {
        ...
        while (!stream.EOS && !stream.currentTokenIs(TokenType.CLOSE_PARENTHESIS)) {
            ...
            let { upperLimit, filter, value } = TableFiltersReader.FilterValue(stream);

            if (stream.currentTokenIs(TokenType.COMMA))
                stream.next();
            tableFilters.push(new FilterCondition(field, filterType, value, upperLimit, filter));
        }
