export default class StringHelper {
  ...
  static groupLines(input: string) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (line[0] === ' ' || line === '') propBuffer.push(line);
      else {
        if (propBuffer.length > 0) properties.push(propBuffer.join('\r\n'));
  ...
  static remove4SpaceIndentation(input: string) {
    return input.replace(/^ {4}/, '').replace(/\r?\n {4}/g, '\r\n');
  }

  static remove2SpaceIndentation(input: string) {
    return input.replace(/^ {2}/, '').replace(/\r?\n {2}/g, '\r\n');
  }
