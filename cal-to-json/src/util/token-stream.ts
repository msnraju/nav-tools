...
export default class TokenStream {
    ...
    ignoreSpaces() {
        let token: IToken = this.current();
        while (token.type == TokenType.SPACE || token.type == TokenType.LINE_BREAK) {
            token = this.next();
        }
    }
    ...
    matchTypePattern(types: TokenType[]) {
        ...
        for (var i = 0; i < types.length; i++) {
            if (this.tokens[this.index + i].type != types[i])
                return false;
        }
    ...
    ascertain(type: TokenType, value: string) {
        ...
        if (token.type != type) {
            throw new TypeError(`token should be of type '${type}' but '${token.type}' found.`);
        }

        if (token.value != value) {
            throw new TypeError(`token value should be '${value}' but '${token.value}' found.`);
        }
    }
    ...
    getString(delimiter: TokenType | IToken | Array<IToken | TokenType>, errorOnEOS: boolean = false) {
        let returnValue = '';
        let token = this.current();
        let parenthesis = 0;

        function isEqual(token: IToken, type: TokenType, value: string | null = null) {
            ...
            if (isArray(delimiter)) {
                for (let i = 0; i < delimiter.length; i++) {
                    ...
                    if ((current as IToken).type) {
                        ...
                        if (isEqual(token, delimiter2.type, delimiter2.value))
                            return true;
                    } else {
                        if (isEqual(token, current as TokenType))
                            return true;
                    }
                }
    ...
    currentTokenIs(type: TokenType, value: string | null = null): boolean {
        const token = this.current();
        if (token && token.type == type && (token.value == value || value == null))
            return true;
